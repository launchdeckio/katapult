...
class Runner {
    ...
    doCommand(command) {
        ...
        return this.context.withScope({command}, subContext => {
            ...
            const operation = this.shouldUseCache(command) ?
            ...
            return doInWorkingDirectory(command.cwd, operation)
        });
    }
     ...
     * execution fails.
     */
    static executeCommand(context, command) {

        return context.getAgent().run(context, command);
    }
