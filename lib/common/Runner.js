...
const finalizeCacheOp         = require('./../report/finalizeCacheOp');
...
class Runner {
    ...
    executeCommand(context, command) {
        ...
        return process.await().then(code => code !== 0 && (() => {

            context.reporter.report({failed: {code}});
            throw new UnexpectedExitCodeError(code);
    ...
    executeCached(context, command) {
        ...
        const onRestore = (cacheableOperation, cachedResult) => context.reporter.report({
            restoring: true,
            created:   cachedResult.created
        });
