...
class Reporter {
     ...
     * @param {RestoredFromCache|SavedToCache} result
     */
    reportCachedOp(result) {
        let msg;
        if (result.runtime)
            msg = sprintf('Restored from cache (extraction took %s)', readableInterval(result.runtime));
        if (result.operationRuntime && result.storageRuntime)
            msg = sprintf('Saved to cache (operation took %s, caching took %s)',
