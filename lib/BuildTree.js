...
class BuildTree {
    ...
    static getConfigFiles(root) {
        ...
        var configFiles = [];
        var ignored     = BuildTree.getGlobalIgnores(root);
        ...
        return fs.listTree(root, (filename, stat) => {
            if (!stat.isDirectory()) {
                ...
                // Return null if the directory is to be ignored so that it will not be traversed
                let ignore = isDirectoryIgnored(filename, ignored);
                return !ignore ? true : null;
            }
        })
            .then(() => configFiles);
    }
