...
class Package extends Action {
    ...
    getAvailableOptions(cli) {
        return _.uniq(_.flatten(_.map(actions, Action => (new Action()).getAvailableOptions(cli))));
    }
    ...
    run(context, args) {
        return Promise.mapSeries(actions, Action => {
            ...
            return action.execute(action.makeContext(args, context.options.cli, context.id), args);
        }).then(results => {
            if (!context.options.cli) return results;
        });
    }
}
