...
class Directory {
    ...
    async clean(context) {
        ...
        if (globs.length) {
            ...
            // We're manually running a safety check here because the
            // "del" module does not take into consideration the `cwd`
            // option when running its built-in safety check.
            assertPathsSafe(globs, true);
