...
const {resolveGlobArray} = require('./common/util/globUtils');
...
class Directory {
    ...
    async clean(context) {
        ...
        let globs = resolveGlobArray(this.buildConfig.purge);

        if (globs.length) {
            
            context.emit[events.CLEAN](this.relativePath);

            // In all cases, append the negating pattern
            // that will prevent build meta files from being deleted
            // This is because "purge" is ran AFTER write-info
            globs = globs.concat(['!.build-{info,sum}*.json']);
            ...
            globs.forEach(glob => {
                if (glob.includes('..'))
                    throw new Error('Please refrain from using ".." in your "purge" globs.');
            });
